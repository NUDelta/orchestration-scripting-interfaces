  ...
  for (const slide of slides) {
    ...
    if (slide.objectId == Canvas[cat].id) {
      for (pageElement of slide.pageElements) {
        ...
        if (
          pageElement.table &&
          pageElement.table.tableRows
          // pageElement.table.tableRows.tableCells
        ) {
          ...
          for (let i = 0; i < num_cols; i++) {
            ...
            // data = cell.text.textElements;
            console.log('COLUMN', i);

            if (
              cell.text &&
              cell.text.textElements
              // cell.text.textElements.textRun
            ) {
              // console.log(cell.text.textElements);
              // console.log('WHEEEEEEEEEEEEEEEE');
              for (const data of cell.text.textElements) {
                if (data.textRun) {
                  console.log('COL', i);
                  console.log(data.textRun.content);
                  jsonData.push(data.textRun.content);
                }

                // Canvas.User_Study_Reflect[`col${i}`] = textContents[i];
              }
