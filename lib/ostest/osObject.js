...
=======
    let DCategory = false, TCategory = false, RCategory = false;
    let pointsD=0, pointsT=0, pointsR = 0;

    let taskObjs = tasks.map(task => {
        console.log(task.taskCategory)
        // Calculate D T R points for the story by summing Task Categories while checking taskCategory
        if (task.taskCategory == 'design'){
            DCategory = true;
            pointsD += task.pointsAllocated
        } else if  (task.taskCategory == 'technology') {
            TCategory = true;
            pointsT += task.pointsAllocated
        } else if (task.taskCategory == 'research') {
            RCategory = true;
            pointsR += task.pointsAllocated;
        } else {
            console.log('task.taskCategory needs to be one of D T and R!')
        }
>>>>>>> 646984ec60ca21806565c13806a88e94cc1efc7d
